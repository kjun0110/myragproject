...
from training.agents.spam_agent.load_model import load_exaone_model
from training.agents.spam_agent.transform_dataset_utils import load_datasets
...
def train_lora_adapter(
    ...
    # 출력 디렉토리 설정 (artifacts 디렉토리 사용)
    # 실제 경로: api/artifacts/exaone/spam_adapter/exaone3.5-2.4b-spam-lora
    if output_dir is None:
        timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        output_dir = api_dir / "artifacts" / "exaone" / "spam_adapter" / "exaone3.5-2.4b-spam-lora" / timestamp
    else:
...
def main():
    ...
    print("\n[2단계] 데이터셋 로드")
    print("-" * 60)
    dataset_dir = training_dir / "data" / "spam_agent" / "exaone"
