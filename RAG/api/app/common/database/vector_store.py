...
이 모듈은 LangChain과 PostgreSQL pgvector 확장을 연결하여
...
try:
    ...
    # 프로젝트 루트 찾기 (api/app/common/database/ -> api/app/ -> api/ -> 프로젝트 루트)
...
def initialize_vector_store(embeddings=None, documents: List[Document] = None) -> PGVector:
    """PGVector 스토어를 초기화합니다.

    Args:
        embeddings: Embedding 모델 (None이면 자동 선택)
        documents: 초기 문서 리스트 (None이면 기본 문서 사용)
    ...
    try:
        ...
        if (
            ...
            try:
                ...
                raise RuntimeError(f"FakeEmbeddings로도 PGVector 스토어 생성 실패: {retry_error}")
        else:
            raise RuntimeError(f"PGVector 스토어 생성 실패: {error_msg[:200]}...")


def test_vector_search(vector_store: PGVector, query: str = "프레임워크", k: int = 2) -> List[Document]:
    """벡터 검색 테스트를 수행합니다.

    Args:
        vector_store: PGVector 스토어
        query: 검색 쿼리
        k: 반환할 결과 수
...
def monitor_vector_store(vector_store: PGVector, interval: int = 30):
    """PGVector 스토어를 주기적으로 모니터링합니다.

    Args:
        vector_store: 모니터링할 PGVector 스토어
        interval: 조회 간격 (초)
    """
    print("\n" + "=" * 50)
            ...
            try:
                ...
                for query in test_queries:
                    try:
                        ...
                        for i, doc in enumerate(query_results[:2], 1):  # 최대 2개만 출력
...
def main() -> None:
    ...
    embeddings = None  # initialize_vector_store에서 자동 선택
    ...
    try:
        query = "프레임워크"
        results = test_vector_search(vector_store, query, k=2)
    ...
    monitor_vector_store(vector_store)
