...
loaders_dir = current_file.parent  # api/app/core/loaders/
core_dir = loaders_dir.parent  # api/app/core/
app_dir = core_dir.parent  # api/app/
...
class ModelLoader:
    ...
    def load_exaone_model(
        ...
        try:
            ...
            if "safetensors" in str(e).lower() or use_safetensors:
                ...
                load_time = time.time() - load_start
                print(
                    f"[INFO] PyTorch pickle 형식으로 로딩 완료 (소요 시간: {load_time:.2f}초)"
                )
            else:
                raise
    ...
    def load_koelectra_model(
        ...
        base_model = AutoModelForSequenceClassification.from_pretrained(model_path, **load_kwargs)
