"""
Schedule Orchestrator - LangGraph 기반 정책/규칙 라우팅
...
class ScheduleOrchestrator:
    """Schedule 데이터 처리 오케스트레이터 (LangGraph 기반)."""
    ...
    async def _determine_strategy_node(self, state: ScheduleProcessingState) -> Dict[str, Any]:
        ...
        logger.info(
            ...
            f"(휴리스틱 기반, JSONL → schedule 테이블 저장)"
        )
    ...
    async def _rule_process_node(self, state: ScheduleProcessingState) -> Dict[str, Any]:
        ...
        try:
            from app.domains.v10.soccer.spokes.services.schedule_service import ScheduleService
    ...
    async def _finalize_node(self, state: ScheduleProcessingState) -> Dict[str, Any]:
        ...
        logger.info("[ORCHESTRATOR] Schedule 처리 완료")
    ...
    def _determine_strategy_koelectra_with_confidence(
        ...
        try:
            ...
            prompt = f"""
다음 Schedule 데이터를 분석하여 정책 기반 처리인지 규칙 기반 처리인지 판단하세요.
...
"""
    ...
    async def process(self, records: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Schedule 레코드들을 처리합니다."""
        logger.info(f"[ORCHESTRATOR] Schedule 처리 시작: {len(records)}개 레코드")
