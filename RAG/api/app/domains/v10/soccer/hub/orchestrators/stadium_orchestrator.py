"""
Stadium Orchestrator - LangGraph 기반 정책/규칙 라우팅
...
class StadiumOrchestrator:
    """Stadium 데이터 처리 오케스트레이터 (LangGraph 기반)."""
    ...
    async def _determine_strategy_node(self, state: StadiumProcessingState) -> Dict[str, Any]:
        ...
        logger.info(
            ...
            f"(휴리스틱 기반, JSONL → stadium 테이블 저장)"
        )
    ...
    async def _rule_process_node(self, state: StadiumProcessingState) -> Dict[str, Any]:
        ...
        try:
            from app.domains.v10.soccer.spokes.services.stadium_service import StadiumService
    ...
    async def _finalize_node(self, state: StadiumProcessingState) -> Dict[str, Any]:
        ...
        logger.info("[ORCHESTRATOR] Stadium 처리 완료")
    ...
    def _determine_strategy_koelectra_with_confidence(
        ...
        try:
            ...
            prompt = f"""
다음 Stadium 데이터를 분석하여 정책 기반 처리인지 규칙 기반 처리인지 판단하세요.
...
"""
    ...
    async def process(self, records: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Stadium 레코드들을 처리합니다."""
        logger.info(f"[ORCHESTRATOR] Stadium 처리 시작: {len(records)}개 레코드")
    ...
    async def process_question(self, question: str) -> Dict[str, Any]:
        ...
        """
        logger.info("=" * 60)
        logger.info("[STADIUM ORCHESTRATOR] 채팅 질문 처리 시작")
        logger.info(f"[STADIUM ORCHESTRATOR] 받은 질문: {question}")
        logger.info("=" * 60)
        
        # 질문을 프린트
        print(f"\n{'='*60}")
        print(f"[STADIUM ORCHESTRATOR] 채팅 질문 수신")
        print(f"[STADIUM ORCHESTRATOR] 질문 내용: {question}")
        print(f"{'='*60}\n")
        
        # KoELECTRA 분류기 어댑터 로드 및 판단
        try:
            logger.info("[STADIUM ORCHESTRATOR] KoELECTRA 분류기 어댑터 로드 시작...")
            ...
            # stadium_orchestrator.py 위치: api/app/domains/v10/soccer/hub/orchestrators/stadium_orchestrator.py
            ...
            if classifier_adapter_base.exists():
                ...
                if subdirs:
                    ...
                    logger.info(f"[STADIUM ORCHESTRATOR] 어댑터 경로: {adapter_path}")
                else:
                    logger.warning(f"[STADIUM ORCHESTRATOR] 어댑터 서브디렉토리를 찾을 수 없습니다: {classifier_adapter_base}")
            else:
                logger.warning(f"[STADIUM ORCHESTRATOR] 어댑터 디렉토리를 찾을 수 없습니다: {classifier_adapter_base}")
            
            # 베이스 모델 로드
            model_path = ModelLoader.KOELECTRA_MODEL_ID
            logger.info(f"[STADIUM ORCHESTRATOR] 베이스 모델 로드: {model_path}")
            ...
            # 어댑터 로드
            if adapter_path:
                logger.info(f"[STADIUM ORCHESTRATOR] 어댑터 로드 중: {adapter_path}")
                classifier_model = PeftModel.from_pretrained(base_model, adapter_path)
                classifier_tokenizer = tokenizer
                logger.info("[STADIUM ORCHESTRATOR] 어댑터 로드 완료")
            else:
                logger.warning("[STADIUM ORCHESTRATOR] 어댑터를 찾을 수 없어 베이스 모델만 사용합니다.")
                classifier_model = base_model
                classifier_tokenizer = tokenizer
            
            classifier_model.eval()
            logger.info("[STADIUM ORCHESTRATOR] KoELECTRA 분류기 어댑터 로드 완료")
            
            # 질문 분류
            logger.info("[STADIUM ORCHESTRATOR] 질문 분류 시작...")
            ...
            # 결과 프린트
            print(f"\n{'='*60}")
            print(f"[STADIUM ORCHESTRATOR] 질문 분류 결과")
            print(f"{'='*60}")
            print(f"질문: {question}")
            ...
            logger.info(f"[STADIUM ORCHESTRATOR] 질문 분류 완료: {predicted_label} (신뢰도: {confidence:.2%})")
            ...
            logger.error(f"[STADIUM ORCHESTRATOR] 분류기 로드/분류 실패: {e}", exc_info=True)
            print(f"\n{'='*60}")
            print(f"[STADIUM ORCHESTRATOR] 분류기 오류")
            print(f"오류: {str(e)}")
            print(f"{'='*60}\n")
        ...
        logger.info(f"[STADIUM ORCHESTRATOR] 채팅 질문 처리 완료")
