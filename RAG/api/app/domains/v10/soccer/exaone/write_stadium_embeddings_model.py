"""
EXAONE이 `stadium_embeddings.py` 파일을 직접 생성/작성하도록 하는 실행 스크립트.

실행 예시 (프로젝트 루트 RAG/ 기준):
  python -m api.app.domains.v10.soccer.exaone.write_stadium_embeddings_model
"""
...
def _validate_generated_python(code: str) -> Tuple[bool, str]:
    ...
    if "class StadiumEmbedding" not in stripped:
        return False, "class StadiumEmbedding 정의가 없습니다."
    if "__tablename__" not in stripped or "stadiums_embeddings" not in stripped:
        return False, "__tablename__ 또는 stadiums_embeddings 지정이 없습니다."
    if "Vector" not in stripped:
        return False, "Vector 사용(또는 import)이 없습니다."
    if "ForeignKey" not in stripped or "stadiums.id" not in stripped:
        return False, "stadiums.id로의 ForeignKey가 없습니다."
    return True, "ok"


def generate_stadium_embeddings_py(
    stadiums_py: str,
    dim: int,
    local_files_only: bool,
) -> str:
    template = f'''"""StadiumEmbedding 모델 (자동 생성)

stadiums 테이블을 기반으로 한 임베딩 테이블 모델입니다.
"""
...
'''
    ...
    prompt = (
        "너는 파이썬 백엔드 개발자다.\n"
        "아래 `stadiums.py`의 Stadium 모델을 참고해서, pgvector 기반 임베딩 테이블 모델 파일을 작성해라.\n"
        "제약:\n"
        "- 출력은 오직 `stadium_embeddings.py` 파일의 전체 내용만.\n"
        "- 설명/마크다운/코드펜스(```) 금지.\n"
        ...
        "----- TEMPLATE END -----\n"
        "\n"
        "이제 `stadium_embeddings.py` 파일 전체 내용을 출력해라.\n"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="EXAONE이 stadium_embeddings.py 파일을 생성/작성")
    ...
    in_path = app_dir / "domains" / "v10" / "soccer" / "models" / "bases" / "stadiums.py"
    out_path = app_dir / "domains" / "v10" / "soccer" / "models" / "bases" / "stadium_embeddings.py"
    ...
    logger.info("[EXAONE] stadium_embeddings.py 생성 중...")
    code = generate_stadium_embeddings_py(
        stadiums_py=_read_text(in_path),
        dim=int(args.dim),
