"""
EXAONE이 `team_embeddings.py` 파일을 직접 생성/작성하도록 하는 실행 스크립트.

실행 예시 (프로젝트 루트 RAG/ 기준):
  python -m api.app.domains.v10.soccer.exaone.write_team_embeddings_model
"""
...
def _validate_generated_python(code: str) -> Tuple[bool, str]:
    ...
    if "class TeamEmbedding" not in stripped:
        return False, "class TeamEmbedding 정의가 없습니다."
    if "__tablename__" not in stripped or "teams_embeddings" not in stripped:
        return False, "__tablename__ 또는 teams_embeddings 지정이 없습니다."
    if "Vector" not in stripped:
        return False, "Vector 사용(또는 import)이 없습니다."
    if "ForeignKey" not in stripped or "teams.id" not in stripped:
        return False, "teams.id로의 ForeignKey가 없습니다."
    return True, "ok"


def generate_team_embeddings_py(
    teams_py: str,
    dim: int,
    local_files_only: bool,
) -> str:
    template = f'''"""TeamEmbedding 모델 (자동 생성)

teams 테이블을 기반으로 한 임베딩 테이블 모델입니다.
"""
...
'''
    ...
    prompt = (
        "너는 파이썬 백엔드 개발자다.\n"
        "아래 `teams.py`의 Team 모델을 참고해서, pgvector 기반 임베딩 테이블 모델 파일을 작성해라.\n"
        "제약:\n"
        "- 출력은 오직 `team_embeddings.py` 파일의 전체 내용만.\n"
        "- 설명/마크다운/코드펜스(```) 금지.\n"
        ...
        "----- TEMPLATE END -----\n"
        "\n"
        "이제 `team_embeddings.py` 파일 전체 내용을 출력해라.\n"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="EXAONE이 team_embeddings.py 파일을 생성/작성")
    ...
    in_path = app_dir / "domains" / "v10" / "soccer" / "models" / "bases" / "teams.py"
    out_path = app_dir / "domains" / "v10" / "soccer" / "models" / "bases" / "team_embeddings.py"
    ...
    logger.info("[EXAONE] team_embeddings.py 생성 중...")
    code = generate_team_embeddings_py(
        teams_py=_read_text(in_path),
        dim=int(args.dim),
