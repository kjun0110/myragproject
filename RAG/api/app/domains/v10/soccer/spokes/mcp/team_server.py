...
MCP_PATH = os.getenv("SOCCER_TEAM_MCP_PATH", "/mcp")
LLM_URL = os.getenv("SOCCER_LLM_MCP_URL", "http://127.0.0.1:9100/mcp")

mcp = FastMCP("Soccer Team Spoke MCP Server")


@mcp.tool
async def team_policy_process(records: List[Dict[str, Any]]) -> Dict[str, Any]:
...
@mcp.tool
async def team_exaone_generate(prompt: str, max_new_tokens: int = 256) -> str:
...
@mcp.tool
async def team_exaone_generate_with_fs_tools(prompt: str, max_steps: int = 5) -> str:
    async with Client(LLM_URL) as c:
        result = await c.call_tool(
