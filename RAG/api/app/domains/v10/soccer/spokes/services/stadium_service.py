...
from app.domains.v10.soccer.hub.repositories.stadium_repository import StadiumRepository

logger = logging.getLogger(__name__)


class StadiumService:
    """Stadium 데이터를 규칙 기반으로 처리하는 Service."""
    
    def __init__(self):
        logger.info("[SERVICE] StadiumService 초기화")
    
    def _validate_record(self, record: Dict[str, Any]) -> Dict[str, Any]:
        """
        Stadium 레코드를 검증하고 변환합니다.
        ...
        """
        # 필수 필드 검증
        required_fields = ["id", "stadium_code", "stadium_name"]
        ...
        validated = {
            ...
            "stadium_name": str(record.get("stadium_name") or record.get("statdium_name")),  # 오타 호환성 유지
            "hometeam_code": str(record["hometeam_code"]) if record.get("hometeam_code") else None,
            "seat_count": safe_int(record.get("seat_count"), "seat_count"),
    ...
    async def process(self, records: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Stadium 레코드들을 규칙 기반으로 처리하고 데이터베이스에 저장합니다.
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                created_stadiums = await repository.create_bulk(validated_records)
                
                # commit 전에 id 추출 (세션 닫힌 후 접근 방지)
                saved_stadium_ids = [s.id for s in created_stadiums]
                
                await repository.commit()
                
                logger.info(
                    f"[SERVICE] 데이터베이스 저장 완료: "
                    f"{len(created_stadiums)}개 레코드 저장됨"
                )
                ...
                result = {
                    ...
                    "saved_records": len(created_stadiums),
                    "validation_errors": validation_errors,
                    "saved_stadium_ids": saved_stadium_ids,
                }
