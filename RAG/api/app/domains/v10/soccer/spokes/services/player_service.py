...
class PlayerService:
    """Player 데이터를 규칙 기반으로 처리하는 Service."""

    def __init__(self):
        logger.info("[SERVICE] PlayerService 초기화")

    def _validate_record(self, record: Dict[str, Any]) -> Dict[str, Any]:
        """
        Player 레코드를 검증하고 변환합니다.
        ...
        """
        # 필수 필드 검증
        required_fields = ["id", "team_code", "player_name"]  # join_yyyy는 null 허용
        ...
        validated = {
            ...
            "team_id": safe_int(record.get("team_id"), "team_id"),
            "team_code": str(record["team_code"]),
            "player_name": str(record["player_name"]),
            "e_player_name": str(record["e_player_name"]) if record.get("e_player_name") else None,
            "nickname": str(record["nickname"]) if record.get("nickname") else None,
            "join_yyyy": str(record["join_yyyy"]) if record.get("join_yyyy") else None,
            "position": str(record["position"]) if record.get("position") else None,
            "back_no": safe_int(record.get("back_no"), "back_no"),
            "nation": str(record["nation"]) if record.get("nation") else None,
            "birth_date": record.get("birth_date"),  # 문자열 또는 None
            "solar": str(record["solar"]) if record.get("solar") else None,
            "height": safe_int(record.get("height"), "height"),
            "weight": safe_int(record.get("weight"), "weight"),
        }
        ...
        # birth_date 문자열을 date 객체로 변환 (나중에 repository에서 처리)
        if validated["birth_date"] and isinstance(validated["birth_date"], str):
            try:
                validated["birth_date"] = date.fromisoformat(validated["birth_date"])
            except ValueError:
                logger.warning(f"잘못된 날짜 형식: {validated['birth_date']}, None으로 설정")
                validated["birth_date"] = None

        return validated

    async def process(self, records: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Player 레코드들을 규칙 기반으로 처리하고 데이터베이스에 저장합니다.
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                created_players = await repository.create_bulk(validated_records)
                await repository.commit()

                logger.info(
                    f"[SERVICE] 데이터베이스 저장 완료: "
                    f"{len(created_players)}개 레코드 저장됨"
                )
                ...
                result = {
                    ...
                    "saved_records": len(created_players),
                    "validation_errors": validation_errors,
                    "saved_player_ids": [p.id for p in created_players],
                }
