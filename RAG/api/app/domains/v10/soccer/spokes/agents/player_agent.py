...
class PlayerAgent:
    ...
    def _build_mcp(self) -> Any:
        ...
        mcp = FastMCP("Player Agent ExaOne")
        agent = self
        ...
        def exaone_generate_with_fs_tools(prompt: str, max_steps: int = 5) -> str:
            ...
            Args:
                prompt: 사용자 요청(예: "api 폴더 파일 목록 보여줘")
                max_steps: 도구 호출 최대 반복 횟수
    ...
    def exaone_generate(self, prompt: str, max_new_tokens: int = 256) -> str:
        """[MCP 툴] ExaOne로 프롬프트에 대한 텍스트를 생성합니다.
            ...
            with torch.no_grad():
                outputs = self.exaone_model.generate(
                    **inputs,
                    ...
                    temperature=0.7,
                    top_p=0.9,
                    pad_token_id=self.exaone_tokenizer.pad_token_id or self.exaone_tokenizer.eos_token_id,
                )
    ...
    def _safe_root(self) -> Path:
        ...
        # player_agent.py 위치: api/app/domains/v10/soccer/spokes/agents/player_agent.py
    ...
    def _fs_tools(self) -> Dict[str, Any]:
        ...
        def join_path(*parts: str) -> str:
            # join 자체는 안전 루트 강제는 하지 않고 문자열 결합만 제공
    ...
    def generate_with_fs_tools(self, user_request: str, max_steps: int = 5) -> Dict[str, Any]:
        ...
        Returns:
            {
              "final": str,
              "steps": [{"tool": str, "args": dict, "result": Any}...]
            }
        """
        ...
        # 대화 히스토리를 문자열로 누적 (간단 구현)
        transcript = f"[SYSTEM]\n{system}\n\n[USER]\n{user_request}\n"

        for _ in range(max_steps):
            raw = self.generate(transcript, max_new_tokens=256)
            raw_json = raw.strip()
            # 모델이 앞뒤로 잡음을 붙이는 경우를 대비해 JSON 블록만 추출 시도
            start = raw_json.find("{")
            end = raw_json.rfind("}")
            ...
            try:
                ...
                # JSON 파싱 실패 → 더 이상 진행 불가로 최종 응답 처리
                return {
