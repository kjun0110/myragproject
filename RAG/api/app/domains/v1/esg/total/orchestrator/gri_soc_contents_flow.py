...
class GRISocContentsOrchestrator(BaseOrchestrator):
    """GRI Social Contents 도메인 오케스트레이터.
    ...
    def classify_domain(
        ...
        try:
            ...
            print(f"[GRI_SOC_ORCHESTRATOR] 정책: {policy}, 도메인: {domain}")
            print(
                f"[GRI_SOC_ORCHESTRATOR] 사용 폴더: {'agents' if use_agents else 'services'}"
            )

            return gate_result

        except Exception as e:
            print(f"[ERROR] GRI Social Contents 오케스트레이터 처리 실패: {e}")
            raise
    ...
    def analyze(
        ...
        if use_agents and policy == "ANALYZE_GRI":
            ...
            print("[GRI_SOC_ORCHESTRATOR] → agents 폴더로 라우팅 (정책 관련)")
            try:
                from ..agents.gri_soc_contents_agent import GRISocContentsAgent

                agent = GRISocContentsAgent()
                agent_result = agent.analyze(text)

                final_decision = f"""[AGENTS 폴더] LLM 기반 GRI Social Contents 분석:
{agent_result.get('response', '분석 완료')}"""
            ...
            print("[GRI_SOC_ORCHESTRATOR] → services 폴더로 라우팅 (규칙 기반)")

            try:
                from ..services.gri_soc_contents_service import GRISocContentsService

                service = GRISocContentsService()
...
def get_gri_soc_contents_orchestrator() -> GRISocContentsOrchestrator:
    """GRI Social Contents 오케스트레이터를 가져옵니다."""
    global _gri_soc_orchestrator
    if _gri_soc_orchestrator is None:
        _gri_soc_orchestrator = GRISocContentsOrchestrator()
    return _gri_soc_orchestrator
