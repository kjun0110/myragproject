"""
GRI Environmental Contents 에이전트 상태 모델 정의.
...
class GRIEnvContentsAgentState(BaseModel):
    """GRI 환경 도메인 콘텐츠 에이전트 상태."""

    # 입력
    query: str = Field(..., description="사용자 쿼리 (환경 도메인 콘텐츠 검색/조회 요청)")
    standard_id: Optional[int] = Field(None, description="GRI 표준 ID")
    disclosure_num: Optional[str] = Field(None, description="공개 번호 (예: '305-1')")
    ...
    contents_list: Optional[list] = Field(None, description="환경 도메인 콘텐츠 목록")
