...
# State 정의 import
from app.domains.v1.chat.models.states.state_model import AgentState
...
def tool_node(state: AgentState):
    ...
    for i, out in enumerate(results):
        tool_messages.append(ToolMessage(content=str(out), tool_call_id=tool_calls[i]["id"]))
...
def run_once(user_text: str):
    ...
    if response_text:
        ...
        response_text = re.sub(
            r"\[\[system\]\].*?\[\[endofturn\]\]\s*", "", response_text, flags=re.DOTALL
        )
        ...
        if any(greeting in user_text.lower() for greeting in ["안녕", "안녕하세요", "hi", "hello"]):
            ...
            for line in lines:
                ...
                if (
                    not line
                    or line.startswith("[")
                    or "system" in line.lower()
                    or "endofturn" in line.lower()
                ):
                    continue
                # 사용자 질문 반복 제거
                if any(
                    greeting in line.lower()
                    for greeting in ["너 이름이", "what's your name", "who are you"]
                ):
                    continue
...
def run_once_with_history(user_message: str, history: list = None):
    """대화 기록을 포함하여 LangGraph 실행.

    Args:
        user_message: 사용자 메시지
        history: 대화 기록 리스트 [{"role": "user", "content": "..."}, ...]
    ...
    # 메시지 리스트 구성
    messages = [SystemMessage(content="You are a helpful assistant. Use tools when needed.")]
    ...
    if response_text:
        ...
        # 3. 간단한 인사에 대한 응답 정리
        if any(greeting in user_message.lower() for greeting in ["안녕", "안녕하세요", "hi", "hello"]):
            # 인사에 대한 간단한 응답만 남기기
            lines = response_text.split("\n")
            clean_lines = []
            for line in lines:
