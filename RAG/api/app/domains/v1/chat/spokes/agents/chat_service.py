...
class ChatService:
    ...
    def initialize_llm(self) -> None:
        ...
        # 로컬 EXAONE LLM 초기화 (LLM_PROVIDER가 exaone인 경우)
        llm_provider = os.getenv("LLM_PROVIDER", "openai").lower()
        if llm_provider == "exaone":
            try:
                from .model_loader import load_exaone_model_for_service

                self.local_llm = load_exaone_model_for_service(
            ...
            print(f"[INFO] LLM_PROVIDER={llm_provider} - EXAONE 모델을 로드하지 않습니다.")
            self.local_llm = None
    ...
    def chat_with_rag(
        ...
        # EXAONE 단일 옵션: 외부에서 "exaone"로 들어오면 내부 "local"로 매핑
            ...
            if any(greeting in message.lower() for greeting in ["안녕", "안녕하세요", "hi", "hello"]):
                ...
                if clean_lines:
                    ...
                    response_text = (
                        ...
                        "궁금한 점이 있거나 도움이 필요한 사항이 있으면 말씀해 주세요. 😊"
                    )
