...
class MidmLLM(BaseLLM):
    ...
    def __init__(
        ...
        Args:
            ...
            torch_dtype: 토치 데이터 타입 ("auto", "float16", "float32" 등)
        ...
        if model_path and Path(model_path).exists():
            ...
            # api/artifacts/midm/midm 확인
            midm_dir = api_dir / "artifacts" / "midm" / "midm"
            if midm_dir.exists() and (midm_dir / "config.json").exists():
                self._load_path = str(midm_dir)
            else:
    ...
    def _load_model(self):
        ...
        print(f"[INFO] Midm 모델 로드 중: {self._load_path}")
    ...
    def get_langchain_model(self) -> BaseChatModel:
        ...
        if self._langchain_model is None:
            ...
            self._langchain_model = HuggingFacePipeline(pipeline=pipe)
