  ...
  const [messages, setMessages] = useState<Message[]>(() => [
    {
      id: "1",
      role: "assistant",
      content: "안녕하세요! OpenAI 모델을 사용하는 LangChain 챗봇입니다. 무엇을 도와드릴까요?",
      timestamp: new Date(),
    },
  ]);
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          const errorMessage: Message = {
            id: (Date.now() + 1).toString(),
            role: "assistant",
            content: "⚠️ OpenAI API 호출량이 초과되었습니다. 할당량을 확인하고 다시 시도해주세요.",
            timestamp: new Date(),
          };
          setMessages((prev) => [...prev, errorMessage]);
          return;
